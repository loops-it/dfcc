...
async function leadFormSubmit() {
  ...
  console.log('Form submitted', formMainID);
  ...
  const response = await fetch("/data-flow-form-data", {
    ...
    body: JSON.stringify({ id: formMainID , inputs: dataFromForm }),
  });
        ...
        try {
          ...
          if (data.productOrService !== null) {
            ...
            const generateHTMLForItem = (item, index) => {
              switch (item.type) {
                ...
                case "formGroup":
                  ...
                  formMainID = item.node_id
                  console.log("type : ", formMainID)
                  return `
            ...
            async function sendNodeId(nodeId) {
              ...
              //           console.log("1 : ",item.node_data[0]?.card)
                  ...
                  .map((item, index) => {
                    switch (item.type) {
                      case "cardGroup":
                        ...
                        const cardGroupHTML = item.node_data.map(node => {
                          const image = node?.card?.image || '';
                          const title = node?.card?.title || '';
                          const description = node?.card?.description || '';
                      ...
                      case "cardStyleOne":
                        ...
                        const cardStyleOneHTML = item.node_data.map(node => {
                          const image = node?.card?.image || '';
                          const title = node?.card?.title || '';
                          const description = node?.card?.description || '';
